...
const Tests = () => {
  ...
  const getStatusBadge = (test) => {
    ...
    if (test.user_status === 'completed') {
      return { text: 'Завершён', class: 'status-completed' };
    }
    if (test.user_status === 'in_progress') {
    ...
    <div className="tests-page">
        ...
        <div className="test-stat-card">
          <div className="test-stat-info">
            <div className="test-stat-value">
              {tests.filter((t) => t.user_status === 'completed').length}
            </div>
      ...
      <div className="tests-grid">
        {tests.map((test) => {
          ...
          const completed = test.completed_attempts ?? 0;
          const hasActive = Boolean(test.has_active_attempt);
          const used = completed + (hasActive ? 1 : 0);
          const maxAtt = test.max_attempts ?? '∞';
          const attemptsText = maxAtt === '∞' ? `${used}/∞` : `${used}/${maxAtt}`;
          const renderAction = () => {
            ...
            return (
              <Link to={`/test/${test.id}`} className="action-btn start-btn">
                {completed > 0 ? 'Пройти заново' : 'Начать тест'}
              </Link>
            );
          };
          return (
            <div key={test.id} className="test-card">
              ...
              <div className="test-content">
                <div className="test-stats-details">
                  ...
                  {test.user_score !== undefined && (
                    <div className="stat-detail">
                      <span className="stat-label">Результат:</span>
                      <span className="stat-value">
                        {test.user_score}/{test.max_score ?? '?'}
                        {test.max_score && (
              ...
              <div className="test-footer">
                <div className="test-actions">{renderAction()}</div>
              </div>
            </div>
          );
        })}
